...
export async function startSidecar({ isPackaged, projectRoot, resourcesPath, userDataPath, env = process.env }) {
  const choice = await choosePort(Number(env.FETCHCV_API_PORT || PREFERRED_PORT));
  const apiBase = `http://${HOST}:${choice.port}`;
  ...
  if (isPackaged) {
    ...
    args = ["--host", HOST, "--port", String(choice.port)];
    ...
    args = ["-m", "uvicorn", "applyos_api.main:app", "--app-dir", "backend", "--host", HOST, "--port", String(choice.port)];
    cwd = projectRoot;
  }
  ...
  return { apiBase, child, ownsProcess: true, port: choice.port };
}
