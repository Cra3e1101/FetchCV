...
export function AgentWorkspace({ detail, runtime, busy, error, activity, task, onStart, onRestart, onFactReview, onReview, onPublishApproval, onRetry, onPauseTask, onResumeTask, onCancelTask, onResolveInvocation, onFinalize, onSendMessage, onResearchInterviews, onCancelMessage, onAddMaterial, onApprovalDecision, onModelActivated, onOpenModelSettings, onSaveResumeEditor, onResumePdfSaved }) {
  ...
  useLayoutEffect(() => {
    ...
    grid.style.setProperty(
      "--context-rail-width",
      railCollapsed ? "46px" : `clamp(268px, ${railWidth}px, min(420px, 28vw))`,
    );
  ...
  return <>
    <main className="main-pane agent-workspace-v2">
      <header className="task-header"><div><span>{detail.job.company}</span><h1>{detail.job.role}</h1></div><nav className="workspace-tabs"><button className={tab === "conversation" ? "active" : ""} onClick={() => setTab("conversation")}><MessageSquare size={14} />对话</button><button className={tab === "resume" ? "active" : ""} onClick={() => setTab("resume")}><FileText size={14} />简历</button><button disabled={!detail.resumes?.length && !detail.interview_briefs?.length} className={tab === "interview" ? "active" : ""} onClick={() => setTab("interview")}><BookOpenCheck size={14} />面试</button></nav><div className={`task-meta runtime-${runtimeState.id}`} title={runtimeState.detail}><i className={runtimeState.active ? "running" : ""} />{runtimeState.active || runtimeState.attention ? runtimeState.label : run ? labels[run.current_stage] || run.status : "尚未开始"}</div></header>
