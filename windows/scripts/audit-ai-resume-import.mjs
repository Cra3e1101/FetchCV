...
const providerStore = process.platform === "win32"
  ? path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "fetchcv-desktop", "settings", "model-provider.json")
  : path.join(os.homedir(), "Library", "Application Support", "fetchcv-desktop", "settings", "model-provider.json");
const resumePath = path.join(root, "test use", "高子强的简历.pdf");
const packagedExecutable = process.platform === "win32"
  ? path.join(root, "release", "win-unpacked", "FetchCV.exe")
  : path.join(root, "release", "FetchCV.app", "Contents", "MacOS", "FetchCV");
